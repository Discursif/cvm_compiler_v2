...
use crate::{BYTE_TYPE, CVMCompCtx, asm::{OperationType}, error::{Paire, ParseError}, expression::Expression, variable::Variable};
use crate::IrAsm;
use crate::Rule;
...
pub enum Instruction {
    ...
    ForRange(Paire<Rule>,Variable, Expression, Expression, Vec<Instruction>),
    ...
    pub fn compile(
        ...
        match self {
            ...
            Instruction::Assign(g,e, a) => {
                ...
                {
                    return Err(ParseError::InvalidAssignement(g.clone(),e.var_type.to_owned(), p.name.to_owned()));
                }
            ...
            Instruction::Return(a,e) => {
                ...
                {
                    return Err(ParseError::InvalidReturnType(a.clone(),function_data.clone(),tp.name.to_owned()))
                }
            ...
            Instruction::ForEach(e,a, b, c) => {
                ...
                Instruction::Assign(e.clone(),object.clone(), b.clone()).compile(ctx, function_data, vars)?;
                Instruction::Assign(e.clone(),
                    objectlen.clone(),
                    Expression::MethodCall(e.clone(),
                ...
                Instruction::Assign(e.clone(),index.clone(), Expression::Value(e.clone(),vec![0])).compile(
                    ctx,
                    function_data,
                    vars,
                )?;
