...
impl Vars {
    ...
    pub fn build(&self) -> Vec<IrAsm> {
        self.merged.iter().map(|x| {
            IrAsm::Cst(x.1[0],x.0.clone())
        }).collect()
    }
    ...
    pub fn remap(&self, input: usize) -> usize {
        if let Some(e) = self.merged.iter().find_map(|(x,y)| {
    ...
    pub fn need_to_be_deleted(&self, input: usize) -> bool {
        self.merged.iter().any(|(_,y)| {
            y.iter().any(|x| x == &input)
        })
    }
}

fn inner_opt(asm: Vec<IrAsm>, consts: &mut Vars) -> Vec<IrAsm> {
    asm.into_iter().flat_map(|x| {
        Some(match x {
            IrAsm::Op(a, b, c, d) => {
                IrAsm::Op(a, b, consts.remap(c), consts.remap(d))
            }
            IrAsm::End => IrAsm::End,
            IrAsm::If(a, b, c, d) => {
                IrAsm::If(consts.remap(a),consts.remap(b), inner_opt(c, &mut *consts), inner_opt(d, &mut *consts))
            }
            ...
            IrAsm::FunctionBlock(a, b) => {
                IrAsm::FunctionBlock(a, inner_opt(b, &mut *consts))
            }
            IrAsm::Return(a) => {
                IrAsm::Return(consts.remap(a))
            }
            IrAsm::Prt(a) => {
                IrAsm::Prt(consts.remap(a))
            }
            IrAsm::Inp(a) => {
                IrAsm::Inp(a)
            }
            ...
            IrAsm::Mov(a, b) => {
                IrAsm::Mov(a, consts.remap(b))
            }
            IrAsm::Len(a, b) => {
                IrAsm::Len(a, consts.remap(b))
            }
            IrAsm::Read(a, b, c, d) => {
